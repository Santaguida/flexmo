<?php
	...
	if(isset($_POST["line"]))
	{
		...
		$query = "SELECT id,enabled FROM tbl_lines WHERE line='" . $line . "'";
		...
		if(mysqli_num_rows($result)<>0)
		{
			...
			if($row['enabled']==0)
			{
				$alert = "This line alreredy exists! But was deleted.
				<br><br>
				<table width='100%'>
				<tr>
				<td class='top_header_link' align='center'>
				<a href='undelete.php?tbl=tbl_lines&col=id&val=" . $row['id'] . "' title='Undelete'>
				[Undelete Line]
				</a>
				</td>
				</tr>
			...
			{
				$alert = "This line alredy exists<br>[Essa linha já existe]";
			}				
		}
		else
		{
			//Insert new product
			$query = "INSERT INTO tbl_lines(line, enabled) VALUES ('" . strtoupper($line) . "','1')";
...
<div id="main_small">
	<div id="div_left_small">
    	<h3 class="blue_title">Manage Existing Lines</h3>
        ...
        <table id="db_list">
        	<tr>
                ...
                <?php
					$query="SELECT id, line 
					FROM tbl_lines
					WHERE enabled=1 ORDER BY line ASC";
					...
					for($j=1;$row=mysqli_fetch_array($result);$j++)
					{
						...
						echo "<tr style='background-color:" .  $color . ";'>
								<td>" . $row['line'] . "</td>
								<td align='center'><a href='delete.php?" . $get_info . "' title='Delete " . $row['line'] . "'><img src='..\images\delete.png' /></a></td>
							  </tr>";
					}				
				?>
        	<tr>
                ...
                <th>
                	<?php
						...
						$query="SELECT line FROM tbl_lines WHERE enabled=1 ORDER BY line ASC";
						...
						$filetype = "LineList"; //file name --> goes between 'flexmo' and 'datetime'
    ...
    <div id="div_right_small"> <!-- This div contains the form to insert new products -->
        <h3 class="blue_title">Insert New Lines</h3>
    
        <form name="line_registration" class="login_form" action="lines.php" method="post">
            <p>
            	<input name="id" type="hidden" value"<?php if(isset($id) && $alert<>""){ echo $id; }?>" />
                <input name="line" type="text" placeholder="Line" value="<?php if(isset($line) && $alert<>""){ echo $line; } ?>" autofocus required>
            </p>
